...
DEFUN(smssubmit, smssubmit_cmd, "smssubmit MS_NAME NUMBER PID DCS MR .LINE",
	"Send an advanced SMS-SUBMIT\nName of MS (see \"show ms\")\nPhone number to send SMS "
	"(Use digits '0123456789*#abc', and '+' to dial international)\n"
	"SMS Protocol Identifier\n"
	"SMS Data Coding Scheme\n"
	"SMS Message Referfence\n"
	"SMS text\n")
{
	struct osmocom_ms *ms;
	struct gsm_settings *set;
	struct gsm_settings_abbrev *abbrev;
	char *number, *pid, *dcs, *mr, *sms_sca = NULL;
	...
	sms_submit(ms, sms_sca, number, pid, dcs, mr, encode_flag, argv_concat(argv, argc, 5));
...
int ms_vty_init(void)
{
	...
	install_element(ENABLE_NODE, &smssubmit_cmd);
