...
struct gsm_sms *sms_submit_from_text(const char *receiver, const char *text,bool encode_flag)
{
	struct gsm_sms *sms = sms_alloc();
	...
	if (encode_flag) {
		...
		memcpy(sms->user_data, sms->text, strlen(sms->text));
		sms->user_data_len = strlen(sms->text);
	}
...
/* create and send advanced SMS-SUBMIT 
 * author: phamsonlam@ginno.com
 */
int sms_submit(struct osmocom_ms *ms, const char *sms_sca, const char *number,
	const char *pid, const char *dcs, const char *msg_ref, bool encode_flag, const char *text)
{
	struct gsm_sms *sms = sms_submit_from_text(number, text, encode_flag);

	if (!sms)
		return -ENOMEM;

	sms->protocol_id = (int)strtol(pid, NULL, 0);
	sms->data_coding_scheme = (int)strtol(dcs, NULL, 0);
	sms->msg_ref = (int)strtol(msg_ref, NULL, 0);

	LOGP(DLSMS, LOGL_DEBUG, "Protocol ID : %d \nData Coding Scheme: %d \nMessage Reference: %d", sms->protocol_id, sms->data_coding_scheme, sms->msg_ref);
